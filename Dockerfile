...
ENV VARNISH_STATIC_TTL="6h"
ENV VARNISH_BERESP_GRACE="120s"
ENV VARNISH_BERESP_KEEP="120s"

CMD ["-p", "thread_pools=8", "-p", "thread_pool_timeout=120", "-p", "thread_pool_add_delay=0.002", "-p", "ban_lurker_sleep=0.1", "-p", "send_timeout=3600", "-p", "connect_timeout=2s", "-p", "first_byte_timeout=480s", "-p", "between_bytes_timeout=60s"]
